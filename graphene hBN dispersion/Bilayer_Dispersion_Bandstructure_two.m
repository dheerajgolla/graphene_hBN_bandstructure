...
ylabel('Energy (meV)')
hold off
dlmwrite('conductionband.txt', energy7);
type conductionband.txt;
dlmwrite('valenceband.txt', energy6);
type valenceband.txt;
...
theta = atan((cos(phi)-1)/sin(phi));
G1x = (4*pi/(sqrt(3)*lambda))*sin(theta+pi/6);
G1y = (4*pi/(sqrt(3)*lambda))*cos(theta+pi/6);
